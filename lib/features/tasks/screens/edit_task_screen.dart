...
class _EditTaskScreenState extends State<EditTaskScreen> {
  ...
  void _update_task() {
    ...
    final updatedTask = TaskModel(
      ...
      description: _desc_controller.text.trim(),
      deadline_date: _format_date(_selected_date!),
      deadline_time: "${_format_time(_selected_time!)}:00",
      is_finished: !_is_pending,
      priority: _is_priority ? 'high' : 'medium',
      attachments: widget.task.attachments,
    );

    context.read<TasksBloc>().add(UpdateTask(id: widget.task.id, task: updatedTask));
